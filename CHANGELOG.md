...
- Current ownership has one documented lifecycle on company and buyer tables: inspect the default deferred `ownership` or `buyer_ownership` output, narrow first when appropriate, populate it, check coverage, then filter or analyze. Buyer Ownership classifies the organization identified by `buyer_url`; transaction tables remain unsupported, and generic research is reserved for ownership questions outside the schema.
